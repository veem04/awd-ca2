...
<div class='mx-auto' style='width: 1000px'>
    <h1 class='text-2xl font-bold my-3'>Edit game</h3>
    ...
    <form action='{{ route('admin.games.update', $game->id) }}' method='post' enctype='multipart/form-data'>
        ...
        <div class='mt-3'>
            ...
            <select class='form-select w-50 rounded-md' name='publisher'>
                <option {{ !$game->publisher ? 'selected' : '' }}>---</option>
                @forelse($publishers as $publisher)
                <option 
                value="{{ $publisher->id }}"
                {{old('publisher') == $publisher->id ? 'selected' : ''}}
                @if($game->publisher)
                {{!old('publisher') && $publisher->id === $game->publisher->id ? "selected" : ''}}
                @endif
                >
                    {{ $publisher->name }}
                </option>
        ...
        <div class='mt-3'>
            ...
            // Prepares all the current genres to have checked by default
            ...
            <ul class='list-group'>
                ...
                <ul class="list-group list-group-horizontal">
                    ...
                    <li class="list-group-item w-100 py-3">
                        ...
                        @else
                        {{in_array($genre->id, $genreIds) ? 'checked' : ''}}
                        @endif
                        >
